...
class GestionBoutiquier extends StatefulWidget {
  const GestionBoutiquier({super.key});

  @override
  State<GestionBoutiquier> createState() => _GestionBoutiquierState();
}

class _GestionBoutiquierState extends State<GestionBoutiquier> {
              ...
              SizedBox(
                ...
                child: ElevatedButton(onPressed: () {
                  ...
                  if( _formKey.currentState!.validate() && _poidsC.text != '' && _gazC.text != '' ){// PERMET DE CONTROLER LES SAISIES DU USER AVANT DE TRAITER LES DATAS                   
                    _quantiteC.text = _quantiteC.text.trim(); // SUPPRIMER LES ESPACES AUX EXTREMITEs DE LA DATA SAISIE
                    setState(() {// SIGNALE QU'IL N'Y A PAS D'ERREUR DE SAISIE (NIVEAU POIDS ET GAZ) DONC N'ENVOIE PAS DE MESSAGE D'ERREUR
                      _choixP=true;
                      _choixG=true;
                    });
                    debugPrint(_gazC.text+_poidsValueSelected);
                    showDialog(
                      context: context, 
                      builder: (contexte) {
                        return AlertDialog(
                ...
                }, child:const Text("Valider",style: TextStyle(fontSize:25),)),
              )
            ],
          ),
